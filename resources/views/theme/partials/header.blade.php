        ...
        <nav class="navbar navbar-expand-lg navbar-light">
            <div class="container box_1620">
                ...
                <div class="collapse navbar-collapse offset" id="navbarSupportedContent">
                        ...
                        <li class="nav-item submenu dropdown @yield('category-active')">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li class="nav-item"><a class="nav-link"
                                        href="{{ route('theme.category', $category->id) }}">{{
                                        $category->name }}</a>
                                </li>
                                @endforeach
                                @endif
                    ...
                    <ul class="nav navbar-nav navbar-right navbar-social text-nowrap">
                        @if (!Auth::check())
                        <a href="{{ route('register') }}" class="btn btn-sm btn-warning">Register / Login</a>
                        @else
                        <li class="nav-item submenu dropdown w-100 flex-nowrap">
                            <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown" role="button"
                                aria-haspopup="true" aria-expanded="false">{{ Auth::user()->name }}</a>
                            <ul class="dropdown-menu">
                                <li class="nav-item"><a class="nav-link" href="{{ route('blog.index') }}">My Blogs</a>
                                </li>
                                <form action="{{ route('logout') }}" method="post" id='logout'>
                                    @csrf
                                    <li class="nav-item"><a class="nav-link"
                                            href="javascript:$('form#logout').submit();">Logout</a></li>
                                </form>
                            </ul>
                        </li>
                        @endif
